...
class KuBoardService:
    ...
    def create_namespace(self, cluster_id: str, namespace: str) -> bool:
        ...
        try:
            ...
            if response.status_code == 201:
                ...
                # 其他错误，需要重试
                error_msg = (
                    f"创建命名空间失败: HTTP {response.status_code}, "
                    f"{response.text}"
                )
                print(f"[KuBoardService] {error_msg}")
    ...
    def grant_permission(
        ...
        print(
            f"[KuBoardService] 授权成功: {cluster_id=} {namespace=} "
            f"{username=} {role=}"
        )
        return True
    ...
    def _grant_stage1_permission(self, cluster_id: str, username: str) -> None:
        ...
        """
        url = (
            f"{self.base_url}/kuboard-api/cluster/{cluster_id}/"
            f"kind/KuboardAuthClusterRoleBinding"
        )
        payload = {
            "kind": "KuboardAuthClusterRoleBinding",
            "metadata": {"cluster": cluster_id, "name": f"user.{username}.viewer"},
            "spec": {
                "subject": {"kind": "KuboardAuthUser", "name": username},
                "role": {"name": "viewer"},
            },
        }
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"[KuBoardService] 第一阶段授权已存在: "
                    f"{cluster_id=} {username=}"
                )
            else:
                error_msg = (
                    f"第一阶段授权失败: HTTP {response.status_code}, "
                    f"{response.text}"
                )
                print(f"[KuBoardService] {error_msg}")
    ...
    def _grant_stage2_permission(
        ...
        try:
            ...
            if response.status_code == 201:
                ...
                error_msg = (
                    f"第二阶段授权失败: HTTP {response.status_code}, "
                    f"{response.text}"
                )
                print(f"[KuBoardService] {error_msg}")
                raise KuboardAuthError(error_msg)
