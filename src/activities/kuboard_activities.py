...
async def grant_permission_activity(params: GrantPermissionParams) -> bool:
    try:
        ...
        service.grant_permission(
            params.cluster_id, params.namespace, params.ldap_user_name, params.role
        )
    ...
    try:
        ...
        for namespace in params.namespaces:
            try:
                ...
                errors.append({"namespace": namespace, "error": f"处理失败: {str(e)}"})

        # 如果有错误，抛出包含详细信息的异常
        if errors:
            error_summary = f"批量处理完成，成功: {len(results)}，失败: {len(errors)}。"
            error_details = "; ".join(
                [f"{err['namespace']}: {err['error']}" for err in errors]
            )
            raise Exception(f"{error_summary} 失败详情: {error_details}")
