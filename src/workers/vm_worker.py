...
async def main(temporal_host: str, temporal_port: int):
    ...
    try:
        # 连接到Temporal服务
        logger.debug(
            f"尝试连接Temporal服务: {temporal_host}:{temporal_port}，超时时间10秒"
        )
        # 设置连接超时为10秒
        try:
            logger.debug(
                f"开始连接Temporal服务: {temporal_host}:{temporal_port}，超时10秒"
            )
            # 简化连接配置，使用默认传输层
            # 添加10秒连接超时控制
        ...
        try:
            ...
            async with Worker(
                ...
                logger.info(
                    f"Temporal Worker已启动，正在监听任务队列: {TEMPORAL_TASK_QUEUE}"
                )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Temporal Worker for VMware VM creation workflow"
    )
    parser.add_argument(
        "--temporal-host",
