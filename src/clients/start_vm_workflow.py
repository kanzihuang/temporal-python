...
async def start_vm_workflow(
    request: VMRequest, temporal_host: str, temporal_port: int
) -> str:
    """启动虚拟机创建工作流"""
    # 连接到Temporal服务
...
def main():
    ...
    parser.add_argument(
        "--power-on", action="store_true", default=True, help="创建后开机"
    )
    ...
    try:
        result = asyncio.run(
            start_vm_workflow(vm_request, args.temporal_host, args.temporal_port)
        )
