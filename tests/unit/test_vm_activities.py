"""
Unit tests for src.activities.vm_activities module.
"""
...
class TestVMActivities:
    ...
    @patch("src.activities.vm_activities.logger")
    async def test_create_vm_activity_success(
        self, mock_logger, mock_vmware_service_class, sample_vm_request
    ):
        """测试成功创建VM活动"""
        # 模拟VMwareService
    ...
    @patch("src.activities.vm_activities.VMwareService")
    @patch("src.activities.vm_activities.logger")
    async def test_create_vm_activity_failure(
        self, mock_logger, mock_vmware_service_class, sample_vm_request
    ):
        """测试VM创建活动失败"""
        # 模拟VMwareService抛出异常
        ...
        mock_logger.info.assert_called_once_with(
            f"开始创建虚拟机: {sample_vm_request.vm_name}"
        )
    ...
    @patch("src.activities.vm_activities.VMwareService")
    @patch("src.activities.vm_activities.logger")
    async def test_create_vm_activity_with_different_vm_names(
        self, mock_logger, mock_vmware_service_class
    ):
        """测试不同VM名称的创建活动"""
        ...
        for vm_name in vm_names:
            # 创建请求
            request = VMRequest(
                vm_name=vm_name, num_cpus=2, memory_gb=4, disk_size_gb=40
            )
    ...
    @patch("src.activities.vm_activities.VMwareService")
    @patch("src.activities.vm_activities.logger")
    async def test_create_vm_activity_with_different_configurations(
        self, mock_logger, mock_vmware_service_class
    ):
        """测试不同配置的VM创建活动"""
        configurations = [
        ...
        for config in configurations:
            # 创建请求
            request = VMRequest(vm_name="test-vm", **config)
    ...
    @patch("src.activities.vm_activities.VMwareService")
    @patch("src.activities.vm_activities.logger")
    async def test_create_vm_activity_with_notes(
        self, mock_logger, mock_vmware_service_class
    ):
        """测试带注释的VM创建活动"""
        request = VMRequest(
    ...
    @patch("src.activities.vm_activities.VMwareService")
    @patch("src.activities.vm_activities.logger")
    async def test_create_vm_activity_without_power_on(
        self, mock_logger, mock_vmware_service_class
    ):
        """测试不自动开机的VM创建活动"""
        request = VMRequest(
    ...
    @patch("src.activities.vm_activities.VMwareService")
    @patch("src.activities.vm_activities.logger")
    async def test_create_vm_activity_vmware_service_exception(
        self, mock_logger, mock_vmware_service_class, sample_vm_request
    ):
    ...
    @patch("src.activities.vm_activities.VMwareService")
    @patch("src.activities.vm_activities.logger")
    async def test_create_vm_activity_logging_levels(
        self, mock_logger, mock_vmware_service_class, sample_vm_request
    ):
        """测试不同日志级别的记录"""
        # 模拟成功情况
    ...
    @patch("src.activities.vm_activities.VMwareService")
    @patch("src.activities.vm_activities.logger")
    async def test_create_vm_activity_error_logging(
        self, mock_logger, mock_vmware_service_class, sample_vm_request
    ):
        """测试错误日志记录"""
        # 模拟失败情况
