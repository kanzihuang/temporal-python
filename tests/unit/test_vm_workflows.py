...
class TestVMCreationWorkflow:
    ...
    @patch(
        "src.workflows.vm_workflows.workflow.execute_activity", new_callable=AsyncMock
    )
    ...
    async def test_run_with_different_vm_requests(self, mock_execute_activity):
        ...
        vm_requests = [
            ...
            VMRequest(
                vm_name="prod-server", num_cpus=8, memory_gb=16, disk_size_gb=200
            ),
    ...
    @patch("src.workflows.vm_workflows.workflow.execute_activity")
    async def test_run_with_retry_policy(
        self, mock_execute_activity, sample_vm_request
    ):
        """测试重试策略配置"""
        mock_execute_activity.return_value = "test-vm-01"
    ...
    @patch("src.workflows.vm_workflows.workflow.execute_activity")
    async def test_run_timeout_configuration(
        self, mock_execute_activity, sample_vm_request
    ):
        """测试超时配置"""
        mock_execute_activity.return_value = "test-vm-01"
    ...
    @patch("src.workflows.vm_workflows.workflow.execute_activity")
    async def test_run_task_queue_configuration(
        self, mock_execute_activity, sample_vm_request
    ):
        """测试任务队列配置"""
        mock_execute_activity.return_value = "test-vm-01"
    ...
    @patch("src.workflows.vm_workflows.workflow.execute_activity")
    async def test_run_activity_returns_different_vm_name(
        self, mock_execute_activity, sample_vm_request
    ):
        """测试活动返回不同的VM名称"""
        # 模拟活动返回不同的VM名称
    ...
    @patch("src.workflows.vm_workflows.workflow.execute_activity")
    async def test_run_multiple_executions(
        self, mock_execute_activity, sample_vm_request
    ):
        """测试多次执行工作流"""
        mock_execute_activity.return_value = "test-vm-01"
    ...
    async def test_run_with_minimal_vm_request(self, mock_execute_activity):
        ...
        minimal_request = VMRequest(
            vm_name="minimal-vm", num_cpus=1, memory_gb=1, disk_size_gb=10
        )
